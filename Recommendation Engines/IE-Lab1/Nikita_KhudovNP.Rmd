...
```{r fig.width=7, fig.height=6}
watch <- 1
watchSW <- which(!is.na(data[,which(names(data) == c("Star.Wars.IV...A.New.Hope"))]))
for (w in c(1:length(data))) {
  watch[w] <- length(which(!is.na(data[watchSW,w])))/length(watchSW)
}
watch <- watch[-1]
watch_list <- as.data.frame(cbind(watch, films))
watch_list <- watch_list[-1,]
watch_list <- watch_list[order(watch_list$watch, decreasing = T),]
watch_list[1:5,]
```
...
```{r fig.width=7, fig.height=6}
like <- 1
likeBabe <- which(data[,"Babe"] >= 4)
for (l in 1:length(data)) {
  like[l] <- mean(data[likeBabe,l], na.rm = T)
}
like <- like[-1]
like_list <- as.data.frame(cbind(like, films))
like_list <- like_list[-20,]
like_list <- like_list[order(like_list$like, decreasing = T),]
like_list[1:5,]
```
